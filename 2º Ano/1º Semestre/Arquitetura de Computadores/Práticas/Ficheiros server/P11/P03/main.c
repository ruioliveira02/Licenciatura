...
//  REMINDER
//
//  These 2 below are the base events and should always be 
//  in elemnts [0] and [1] of Events
//  PAPI_TOT_CYC - Total processor clock cycles
//  PAPI_TOT_INS - Total instructions completed
//
//  The following are additional events, which can be in indexes >=2 
//  of Events
//  Measuring these events does currently require rerunning 
//  the whole measuring loop
//
...
#define NUM_EVENTS 5
int Events[NUM_EVENTS] = { PAPI_TOT_CYC, PAPI_TOT_INS, PAPI_SR_INS, PAPI_LD_INS, PAPI_L1_DCM};
...
int main (int argc, char *argv[]) {
  ...
  NUM_RUNS = ( version<=1 ? 2 : 3);
  if (NUM_EVENTS>2) NUM_RUNS=1;

  for (measurement_loop = 0 ; measurement_loop <= NUM_EVENTS-2 ; measurement_loop++)
  {
    fprintf (stderr, ">>>>\n\tMeasurement loop %d out of %d\n", measurement_loop, NUM_EVENTS-2);
    // warmup caches
